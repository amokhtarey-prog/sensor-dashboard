...
/* ---------------------------- card rendering ---------------------------- */
const GAUGE_RANGES = {
  accel: [0, 20], gyro: [0, 400], magneto: [0, 150], compass: [0, 360],
  light: [0, 1000], pressure: [950, 1050], devicemotion: [0, 15],
  altitude: [-50, 3000], speed: [0, 30], battery: [0, 100],
};

function buildCardDOM(def) {
  const el = document.createElement('div');
  el.className = 'card' + (def.id === 'battery' ? ' card-battery' : '');
  el.id = `card-${def.id}`;
  const showBigValue = def.id !== 'battery';
  el.innerHTML = `
    <div class="card-top">
      <span class="card-name">${def.name}</span>
      <span class="status-tag ${STATUS_CLASS.checking}" data-role="status">${STATUS_LABEL.checking}</span>
    </div>
    ${showBigValue ? `<div><span class="card-value" data-role="value">--</span><span class="card-unit">${def.unit}</span></div>` : ''}
    <div class="card-sub" data-role="sub"></div>
    <div class="card-stats" data-role="stats"></div>
    ${def.chart ? '<canvas data-role="chart"></canvas>' : ''}
  `;
  return el;
}
...
function renderCard(id) {
  ...
  if (s.status !== 'available' && s.status !== 'simulated') {
    ...
    statsEl.textContent = '';
    if (id === 'battery') { const c = el.querySelector('[data-role="chart"]'); if (c) ChartManager.drawLiquidBattery(c, null); }
    return;
  }
  if (valueEl) valueEl.textContent = (s.primary !== null && s.primary !== undefined) ? fmt(s.primary, Math.abs(s.primary) < 10 ? 3 : 2) : (Object.values(s.fields)[0] ?? '--');
  ...
  if (s.chart) {
    ...
    if (id === 'battery') {
      ChartManager.drawLiquidBattery(canvas, s.primary);
    } else {
      const range = GAUGE_RANGES[id];
      ChartManager.drawGauge(canvas, s.primary, range ? range[0] : s.min, range ? range[1] : s.max);
    }
  }
}
